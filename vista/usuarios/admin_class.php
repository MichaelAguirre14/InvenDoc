...
$data = "";
foreach($_POST as $k => $v){
	if(!in_array($k, array('id','table')) && !is_numeric($k)){
		if(empty($data)){
			$data .= " $k='$v' ";
		}else{
			$data .= ", $k='$v' ";
...
$check = $this->db->query("SELECT * FROM customers WHERE email ='$email' ".(!empty($id) ? " AND id != $id " : ''))->num_rows;
if($check > 0){
	return 2; // Error: El correo electrónico ya existe para otro cliente.
	exit;
}
if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
	$fname = strtotime(date('y-m-d H:i')).'_'.$_FILES['img']['name'];
...
if($save){
	return 1; // Éxito: El cliente se actualizó correctamente.
}
}
	...
	//Funcion Borrar el Pedido
	function delete_pedido() {
		extract($_POST);
		$delete = $this->db->query("DELETE FROM pedidos WHERE id = $pedido_id");
		if ($delete) {
			return 1;
		}
	}
